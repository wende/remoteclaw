...
- [Final Step: Print the MCP Client Config](#final-step-print-the-mcp-client-config)
...
**Requires interactive terminal access** — `mkcert -install` prompts for a sudo password to install the CA into the system trust store. If you are an AI agent, hand this step off to the user rather than trying to run it non-interactively.
...
**Wait at least 8-10 seconds** after restart for the plugin service to initialize. Dynamic tool discovery imports the full OpenClaw tool graph, which takes a moment on first load. If a health check immediately after restart returns "connection refused", that's normal — retry after a few seconds rather than assuming failure. A good pattern:
...
openclaw gateway install
sleep 8
curl http://127.0.0.1:3100/health || (sleep 5 && curl http://127.0.0.1:3100/health)
```
...
> **CRITICAL: Do NOT write the config until you have the real tunnel URL.**
>
> The `issuerUrl` field is parsed as a URL on plugin startup. Setting it to a placeholder value
> (e.g., `"TUNNEL_URL_PLACEHOLDER"`, `"TODO"`, or any non-URL string) will cause a `TypeError: Invalid URL`
> crash and the plugin service will fail to start. **Always start the tunnel first**, copy the real URL
> (e.g., `https://random-words.trycloudflare.com`), and only then write the config and restart the gateway.
...
- **Static catalog (normal path)**: The file `src/tool-catalog.json` is already committed to the RemoteClaw repo. If it exists (check with `ls src/tool-catalog.json`), the plugin should find tools automatically. **You do NOT need to clone the OpenClaw source tree or run `npm run refresh-catalog`** — the catalog is pre-built and shipped with the repo.
- **Dynamic discovery (advanced)**: If the static catalog is missing or you want live tool discovery, it requires the OpenClaw dev source at `~/projects/openclaw` (or set `openclawRoot` in plugin config, or set `OPENCLAW_ROOT` env var). The source must have `src/agents/openclaw-tools.ts`. This is only needed for development — not for normal setup.
- **If `src/tool-catalog.json` is missing**: Run `npm run refresh-catalog` to regenerate it, but this requires the OpenClaw dev source with a `node_modules/`. Symlink it from the installed package if needed: `ln -s $(dirname $(which openclaw))/../lib/node_modules/openclaw/node_modules ~/projects/openclaw/node_modules`

### Plugin crashes with `TypeError: Invalid URL`

**Cause**: The `issuerUrl` field in the plugin config is set to a non-URL value (placeholder, empty string, or malformed URL). The plugin parses this on startup and crashes immediately.

**Fix**: Ensure `issuerUrl` is a valid HTTPS URL (e.g., `https://random-words.trycloudflare.com`). If you don't have the tunnel URL yet, omit `issuerUrl` entirely from the config — don't use a placeholder. Start the tunnel first, get the real URL, then update the config and restart.
...
## Final Step: Print the MCP Client Config

**Always end the setup by printing the ready-to-use MCP client configuration** so the user can copy-paste it directly. Choose the appropriate variant based on the setup choices made earlier.
...
Ensure NODE_EXTRA_CA_CERTS is set in your shell before launching Claude Code
(see Step 3d).
```

**This is mandatory** — every setup run must end with this output. The user should be able to take the printed config and immediately use it without referring back to the guide.
